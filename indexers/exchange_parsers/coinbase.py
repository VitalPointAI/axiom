...
class CoinbaseParser(BaseExchangeParser):
    ...
    def detect(self, filepath: str, first_lines: list) -> bool:
        """Return True if this looks like a Coinbase CSV export."""
        header = " ".join(first_lines[:2]).lower() if first_lines else ""
        return "timestamp" in header and "transaction type" in header
    ...
    def parse_row(self, row):
        ...
        quantity = (
            row.get("Quantity Transacted")
            or row.get("quantity_transacted")
            or row.get("Amount")
        )
